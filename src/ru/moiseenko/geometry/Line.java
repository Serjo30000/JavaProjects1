 ...
 * @param <T>
 */
public class Line<T extends Pointable> implements Lenable, Cloneable,Shiftable{
    ...
    public double lenLom(){
        double ln=0;
        if (startP.getClass()==Point.class && endP.getClass()==Point.class){
            ln = Math.sqrt((getStart().x - getEnd().x)*(getStart().x - getEnd().x)+(getStart().y - getEnd().y)*(getStart().y - getEnd().y));
        }
        if (startP.getClass()==Point3d.class && endP.getClass()==Point3d.class){
            ln = Math.sqrt((((Point3d)(getStart())).x - ((Point3d)(getEnd())).x)*(((Point3d)(getStart())).x - ((Point3d)(getEnd())).x)+(((Point3d)(getEnd())).y - ((Point3d)(getEnd())).y)*(((Point3d)(getStart())).y - ((Point3d)(getEnd())).y)+(((Point3d)(getStart())).z - ((Point3d)(getEnd())).z)*(((Point3d)(getStart())).z - ((Point3d)(getEnd())).z));
        }
        return ln;
    }
    @Override
    ...
    public void shiftCoordinate(String nameC, int c) {
        if (nameC.equals("x")){
            if (startP.getClass()==Point.class && endP.getClass()==Point.class){
                getStart().x=getStart().x+c;
                getEnd().x=getEnd().x+c;
            }
        }
        else if (nameC.equals("y")){
            if (startP.getClass()==Point.class && endP.getClass()==Point.class){
                getStart().y=getStart().y+c;
                getEnd().y=getEnd().y+c;
            }
        }
    }
